...
(function () {
  ...
  function selectState (selectedState) {
    for (const state in gameSettings.states) {
  ...
  function checkObstaclePosition (obstacle, positionToCheck) {
    if (positionToCheck === position.any) {
      const index = Object.values(CURRENT_PATH).findIndex(value => value === obstacle)
  ...
  function animationCharacter () {
    ...
    const totalFrames = homer[currentAnimation].frames // Cambiar homer por template String ${} cuando se pueda elegir el pj dinámicamente.
  ...
  function drawTree (x, y, width, height) {
    ...
    x = perspective * x * 0.8
    y /= perspective * 5.8
    width = perspective * width * 0.8
    height = perspective * height * 0.8
